...
class TrussController():
    ...
    # -------------------------------------------------------------------
    # ADDED: These new methods allow the GUI (MainWindow) to talk
    # to the scene through the controller (instead of view.scene).
    # -------------------------------------------------------------------
    def installEventFilter(self, filterObj):
        """Attach an event filter to the scene via the controller."""
        self.view.scene.installEventFilter(filterObj)  # calls view

    def getScene(self):
        """Return the underlying scene so that eventFilter can compare obj."""
        return self.view.scene

    def sceneItemAt(self, pos, transform):
        """Return the item at the given scene position, using the given transform."""
        return self.view.scene.itemAt(pos, transform)

    def sceneItemsAt(self, pos):
        """Return all items at the given scene position."""
