...
class MainWindow(Ui_TrussStructuralDesign,qtw.QWidget):
    def __init__(self):
        ...
        # --- JES Missing Code: This calls the function from the view directly. ---
        # --- Fix so that it only calls the controller directly. ---
        # Original: self.controller.view.scene.installEventFilter(self)
        self.controller.installEventFilter(self)  # CHANGED
    ...
    def eventFilter(self, obj, event):
        ...
        if obj == self.controller.view.scene:
            ...
            if et == qtc.QEvent.GraphicsSceneMouseMove:
                ...
                # Original: s = self.controller.view.scene.itemAt(scenePos, self.gv_Main.transform())
                s = self.controller.sceneItemAt(scenePos, self.gv_Main.transform())  # CHANGED

                if s is not None and s.data(0) is not None:
                    strScene += ' (' + s.data(0) + ')'

                # Original: items = self.controller.view.scene.items(event.scenePos())
                items = self.controller.sceneItemsAt(event.scenePos())  # CHANGED
